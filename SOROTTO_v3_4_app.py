...
let ws=null,role=null,code=null,hostToken=null,playerId=null,playerToken=null,currentOrder=[],chatDraft='',lastHostStage=null,lastHostState=null,lastPlayerState=null;
...
function stateCore(s){if(!s)return '';const x={...s};delete x.chat;return JSON.stringify(x)}
function chatMessagesHtml(s){return (s.chat||[]).map(m=>m.kind==='system'?`<div class="chatMsg system">${esc(m.text)}</div>`:`<div class="chatMsg"><div class="chatHead">${esc(m.sender)}</div><div class="chatText">${esc(m.text)}</div></div>`).join('')||'<div class="chatMsg system">まだメッセージはありません</div>'}
function updateChatOnly(root,s){const box=root?.querySelector('#chatBox');if(!box)return false;box.innerHTML=`${chatMessagesHtml(s)}<div id="chatEnd" aria-hidden="true"></div>`;scrollChatToLatest(root);return true}
function connect(r){if(ws)try{ws.close()}catch{};const proto=location.protocol==='https:'?'wss':'ws';let url=`${proto}://${location.host}/ws/${code}?role=${r}&token=${encodeURIComponent(r==='host'?hostToken:playerToken)}`;if(r==='player')url+=`&player_id=${encodeURIComponent(playerId)}`;ws=new WebSocket(url);ws.onopen=()=>{$('#conn').textContent='LIVE';$('#conn').style.color='#78dfa0'};ws.onclose=()=>{$('#conn').textContent='RECONNECT';$('#conn').style.color='';setTimeout(()=>connect(r),1800)};ws.onerror=()=>toast('接続を確認しています…');ws.onmessage=e=>{const d=JSON.parse(e.data);if(d.type==='error')return toast(d.message);const prev=r==='host'?lastHostState:lastPlayerState;const onlyChat=prev&&stateCore(prev)===stateCore(d)&&JSON.stringify(prev.chat||[])!==JSON.stringify(d.chat||[]);if(r==='host')lastHostState=d;else lastPlayerState=d;if(onlyChat){const root=r==='host'?(d.stage==='lobby'?$('#hostLobby'):$('#hostGame')):$('#playerGame');if(updateChatOnly(root,d))return}if(r==='host')renderHost(d);else renderPlayer(d)}}
...
function renderHost(s){
 ...
 const sameStage=lastHostStage===s.stage, pageY=window.scrollY;
 lastHostStage=s.stage;
 const restorePage=()=>requestAnimationFrame(()=>window.scrollTo(0,sameStage?pageY:0));
 if(s.stage==='lobby'){
   ...
   $('#start').onclick=()=>send('start');bindChat(lobby);restorePage();return;
 }
 lobby.style.display='none';lobby.innerHTML='';g.style.display='';
...
function renderPlayer(s){const g=$('#playerGame');if(s.stage==='lobby'){g.innerHTML=`<div class="card hero center"><div class="eyebrow">ROOM ${s.code}</div><div class="topic">参加完了！</div><p>${esc(s.name)} さん</p><p class="hint">ホストがゲームを始めるまで、この画面で待ってください。</p></div><div class="card"><div class="smallcaps">PLAYERS</div><div class="players">${renderPlayers(s.players)}</div></div>${chatHtml(s)}`;bindChat(g);return}
 if(s.stage==='reveal'){g.innerHTML=`<div class="card"><div class="smallcaps">ROUND ${s.round} / ${s.max_rounds}</div></div>${topicCard(s.topic)}<div class="card secret"><div class="smallcaps">YOUR NUMBER</div><div class="number">${s.number}</div><p class="hint">この数字はあなただけの秘密。数字そのものは言わず、お題に合う“たとえ”で表現してください。</p><div class="pill">みんなが確認中</div></div>${chatHtml(s)}`;bindChat(g);return}
 if(s.stage==='discuss'){g.innerHTML=`<div class="card"><div class="smallcaps">ROUND ${s.round} / ${s.max_rounds}</div></div>${topicCard(s.topic)}<div class="card secret"><div class="smallcaps">YOUR NUMBER</div><div class="number">${s.number}</div><p class="hint">この数字はあなただけの秘密。数字そのものは言わず、お題に合う“たとえ”で表現してください。</p><div class="pill">相談中</div></div>${currentPositionHtml(s,s.player_id)}${chatHtml(s)}`;bindChat(g);return}
 if(s.stage==='result'||s.stage==='finished'){g.innerHTML=`<div class="card"><div class="smallcaps">RESULT</div><div class="topic ${s.perfect?'ok':'bad'}">${s.perfect?'そろった！':'惜しい！'}</div>${resultCardsHtml(s,s.player_id)}</div><div class="card center"><div class="smallcaps">TEAM SCORE</div><div class="score">${s.score}</div><p class="hint">${s.stage==='finished'?'ゲーム終了！おつかれさま！':'ホストが次のラウンドへ進めます。'}</p></div>${chatHtml(s)}`;bindChat(g);return}
}
