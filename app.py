...
function chatHtml(s){const msgs=(s.chat||[]).map(m=>m.kind==='system'?`<div class="chatMsg system">${esc(m.text)}</div>`:`<div class="chatMsg"><div class="chatHead">${esc(m.sender)}</div><div class="chatText">${esc(m.text)}</div></div>`).join('');return `<div class="card"><div class="smallcaps">ROOM CHAT</div><div class="chatBox" id="chatBox">${msgs||'<div class="chatMsg system">まだメッセージはありません</div>'}</div><div class="chatComposer"><input id="chatInput" maxlength="120" inputmode="text" autocomplete="off" placeholder="たとえを送る…" value="${esc(chatDraft)}"><button class="btn secondary" id="chatSend">送信</button></div><div class="reactions"><button class="react" data-react="👍">👍</button><button class="react" data-react="🤔">🤔</button><button class="react" data-react="😂">😂</button><button class="react" data-react="👀">👀</button><button class="react" data-react="⬆️">⬆️</button><button class="react" data-react="⬇️">⬇️</button></div><p class="noDigits">チャットでは数字を含むメッセージは送信できません。</p></div>`}
function bindChat(){const inp=$('#chatInput'),btn=$('#chatSend'),box=$('#chatBox');if(!inp||!btn)return;inp.oninput=()=>chatDraft=inp.value;const submit=()=>{const text=inp.value.trim();if(!text)return;if(/[0-9０-９]/.test(text)){toast('数字はチャットで使えません');return}send('chat',{text});chatDraft='';inp.value=''};btn.onclick=submit;inp.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();submit()}};document.querySelectorAll('[data-react]').forEach(b=>b.onclick=()=>send('react',{emoji:b.dataset.react}));if(box)box.scrollTop=box.scrollHeight}
